...
class BooksExpenses(BooksConnection):

    def __init__(self):
        super().__init__()
        self.headers = {"Authorization": f"Zoho-oauthtoken {self.access_token}"}

    #Create record in Booksexpenses
    def create_record(self, json_client):
        ...
        json_create = requests.post(
            f'https://books.zoho.com/api/v3/expenses/?organization_id={ZBOOKS_ORGANITATION_ID}',
    ...
    def get_records(self, queryparams = {}, page = 1):
        request_query = {
            "url": f'https://books.zoho.com/api/v3/expenses?page={str(page)}&organization_id={ZBOOKS_ORGANITATION_ID}',
        ...
        get_expenses_list_response = requests.get(**request_query)
        expenses_collected = []


        if get_expenses_list_response.status_code == 200:
            expenses_collected = expenses_collected + get_expenses_list_response.json().get("expenses")
            has_more_pages = get_expenses_list_response.json().get("page_context").get("has_more_page")
            actual_page = get_expenses_list_response.json().get("page_context").get("page")

            response_json={
                "expenses": expenses_collected,
                "has_more_pages": has_more_pages,
                "page": actual_page,
                "status_code":get_expenses_list_response.status_code,
                "code":get_expenses_list_response.json().get("code"),
                "message":get_expenses_list_response.json().get("message")
            }

        else:
            response_json = {"expenses": [],
    ...
    def get_record(self,idclient):
        ...
        get_single_contatcs = requests.get(
            f'https://books.zoho.com/api/v3/expenses/{idclient}?organization_id={ZBOOKS_ORGANITATION_ID}', headers=self.headers)
    ...
    def update_record(self,idclient,json_client):
        ...
        PUT /expenses/{contact_id}
       {
        ...
        json_update = requests.put(
            f'https://books.zoho.com/api/v3/expenses/{idclient}?organization_id={ZBOOKS_ORGANITATION_ID}',
            headers=headers, data=json_data)
    ...
    def mark_record(self,idclient,optn):
        ...
        mark_contact = requests.post(
            f'https://books.zoho.com/api/v3/expenses/{idclient}/{urlp[optn]}?organization_id={ZBOOKS_ORGANITATION_ID}',
            headers=self.headers)
    ...
    def enable_portal_access(self,idclient,json_client):
        ...
        requestZBK = requests.post(
            f'https://books.zoho.com/api/v3/expenses/{idclient}/portal/enable?organization_id={ZBOOKS_ORGANITATION_ID}',
            headers=headers, data=json_data)
    ...
    #method to set payment reminder in books expenses
    def payment_reminder(self,idclient,optn):
        ...
        """
        edo=['enable','disable']
        requestZBK= requests.post(f'https://books.zoho.com/api/v3/expenses/{idclient}/paymentreminder/{edo[optn]}?organization_id={ZBOOKS_ORGANITATION_ID}',
                    headers=self.headers)
    ...
    def email_statment(self,jsonmail):
        """
            POST /expenses/{:contact_id}/statements/email
    ...
    def get_statment_mail(self,idclient):
        """
        GET /expenses/{:contact_id}/statements/email
    ...
    def list_comments_expenses(self,idclient,queryparams = {}, page = 1):
        """
        Arguments
            contact_persons | Required | ID of the contact person
        ...
        Return all comments of the expenses
        """
        requestZKB = requests.get(
            f'https://books.zoho.com/api/v3/expenses/{idclient}/comments?organization_id={ZBOOKS_ORGANITATION_ID}', headers=self.headers)
    ...
    def get_contact_address(self,idclient):
        ...
        """
        requestZKB= requests.get(f'https://books.zoho.com/api/v3/expenses/{idclient}/address?organization_id={ZBOOKS_ORGANITATION_ID}',headers=self.headers)
        return requestZKB.json()

    #Return all credit note of expenses
    def list_refunds(self,idclient):
        ...
        """
        requestZKB = requests.get(f'https://books.zoho.com/api/v3/expenses/{idclient}/refunds?organization_id={ZBOOKS_ORGANITATION_ID}',headers=self.headers)
